...
class InputBox:
    ...
    def __init__(self, rect, text='', fill=True, border=False, **kwargs):
        """Creates a pygame button"""
        self.rect = rect
        self.fill = fill
        self.text = text
    ...
    def display(self, displaySurf, events):
        ...
        for event in events:
            if event.type == MOUSEBUTTONUP and self.buttonRect.collidepoint(event.pos[0], event.pos[1]):
                self.active = True if self.active == False else False
            elif event.type == KEYUP:
                if event.unicode == '\r':
                    self.active = False
                elif event.key == K_BACKSPACE and self.active:
                    self.text = self.text[:-1]
            elif event.type == TEXTINPUT and self.active:
